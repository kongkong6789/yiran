...
- 所有接口均要求登录，且调用者需具备管理权限（平台管理员、超级管理员或企业管理员），否则返回 `401` / `403`。

### 两类团队

| 类型 | `kind` | 归属 | 谁可管理 | 成员范围 |
| --- | --- | --- | --- | --- |
| 平台团队 | `platform` | 跨企业，无 `organization` | 平台管理员（`is_staff`）、超级管理员 | 任意启用的平台用户 |
| 企业团队 | `enterprise` | 归属某个企业 | 该企业的所有者/管理员、超级管理员 | 仅该企业的启用成员 |

### 可见范围

- 超级管理员：可见并管理全部团队。
- 平台管理员：可见全部平台团队；对企业团队仅可见其担任管理员的企业。
- 企业管理员：仅可见其担任管理员的企业下的企业团队。
