...
function renderMessageBody(content: string, room: CollabRoom | null, asMarkdown: boolean) {
  ...
  if (asMarkdown || looksLikeMarkdown(content)) {
    return (
      <div className="collab-md blocks">
...
export default function CollabRisk() {
  ...
  const loadRoomDetail = useCallback(async (id: string) => {
    try {
      ...
      try {
        const st = await getCollabRoomStats(id);
        setRoomStats(st);
      } catch {
        setRoomStats(null);
      }
  ...
  const sendPlainMessage = async (content: string, files: File[] = []) => {
    ...
    try {
      ...
      scrollMessagesToBottom("auto");
      if (res.room) {
        setActiveRoom((prev) => (prev ? { ...prev, ...res.room } : prev));
      ...
      <section className="collab-main">
        {!activeRoom ? (
          <div className="collab-empty">
            <CollabRoundTable variant="empty" />
            <Typography.Title level={4} style={{ margin: "4px 0 0", fontWeight: 600 }}>
              协作会话
            </Typography.Title>
                  ...
                  <div className="agent-chat-composer-right">
                    <Tooltip title={`上传附件（最多 ${MAX_FILES} 个）`}>
                      <Button
                        ...
                        shape="circle"
                        icon={<PlusOutlined />}
                        disabled={!isParticipant || activeRoom.status === "closed" || sending || pendingFiles.length >= MAX_FILES}
