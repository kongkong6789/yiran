...
export default function TaskResultPanel({ result }: Props) {
  ...
  const previewArtifact = async (url: string, name: string) => {
    ...
    modal.info({ title: name, width: 820, content: <pre className="task-artifact-preview">{typeof response.data === "string" ? response.data : JSON.stringify(response.data, null, 2)}</pre> });
  };
