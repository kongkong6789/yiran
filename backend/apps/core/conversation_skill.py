...
def prepare_conversation_skill(
    ...
    *,
    ...
    emit_progress(progress_callback, "history_read", "running")
    rows = _conversation_rows(room, exclude_message_id=exclude_message_id)
    emit_progress(progress_callback, "history_read", "completed")
    raise_if_cancelled(cancel_check)
    emit_progress(progress_callback, "redaction", "running")
    rows = [
        {"role": row["role"], "content": _sanitize(row["content"])}
        for row in rows
    ]
    emit_progress(progress_callback, "redaction", "completed")
...
不得虚构对话中未出现的 API、脚本、权限或数据源；不得输出任何凭据或个人敏感信息。"""
    ...
    emit_progress(progress_callback, "skill_summary", "running")
    ...
    emit_progress(progress_callback, "skill_summary", "completed")
    ...
    emit_progress(progress_callback, "package_validation", "running")
    try:
    ...
    emit_progress(progress_callback, "package_validation", "completed")
    raise_if_cancelled(cancel_check)
