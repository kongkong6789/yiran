...
_LLM_DELETE_EXTRACT_SYSTEM = """你是企微待办删除意图解析器。根据用户消息（及必要时的对话历史）判断要删除哪条待办。
只输出一个 JSON 对象，不要 Markdown，不要解释。

字段：
- ok: boolean，能否确定要删除的待办
- title: string，待办标题或能唯一定位待办的关键词（从消息或历史推断）
- assignee_names: string[]，负责人姓名（可选；没有则 []）
- error: string，ok=false 时说明缺什么

规则：
1. 用户说删除/移除/撤销待办时，action 视为删除。
2. 若当前消息很模糊（如「也删除」「把企微的删掉」），必须从【最近对话】里找刚创建或讨论过的待办标题。
3. title 应是事项本身，不要包含「删除」「待办」等动词。
4. 无法唯一定位时不要编造标题，返回 ok=false。
"""

_DELETE_INTENT_RE = re.compile(r"(删除|移除|撤销|去掉|取消|删掉|清除)")
_VAGUE_DELETE_RE = re.compile(r"(也删除|也移除|也撤销|也删掉|那条|这个|上面|刚才|刚刚|企微的|企业微信的)")
...
@dataclass
class ParsedWeComTodoDeleteRequest:
    title_query: str
    assignee_names: list[str]


def _looks_like_delete_intent(message: str, history: list[dict] | None = None) -> bool:
    raw = _clean_message(message)
    if not raw or not _DELETE_INTENT_RE.search(raw):
        return False
    if re.search(r"待办|任务|企微|企业微信|那条|这个|上面|刚才|刚刚|也删|也移|也撤销", raw):
        return True
    if _VAGUE_DELETE_RE.search(raw):
        return True
    if history:
        blob = " ".join(str(item.get("content") or "") for item in history[-6:])
        if "待办" in blob and ("标题" in blob or "企微待办技能执行结果" in blob):
            return True
    return False


def _strip_delete_prefix(raw: str) -> str:
    text = (raw or "").strip()
    text = re.sub(
        r"^(请|麻烦)?(帮我|帮忙|帮)?(我)?(把|将)?(企业微信|企微)?(的)?",
        "",
        text,
        flags=re.IGNORECASE,
    ).strip()
    text = re.sub(
        r"^(删除|移除|撤销|去掉|取消|删掉|清除)(掉|了)?(企业微信|企微)?(的)?(待办|任务)?[：:\s]*",
        "",
        text,
        flags=re.IGNORECASE,
    ).strip()
    text = re.sub(r"(也)?(删除|移除|撤销|删掉|清除)(掉|了)?$", "", text, flags=re.IGNORECASE).strip()
    return text


def _extract_todo_title_from_history(history: list[dict] | None) -> str:
    if not history:
        return ""
    for item in reversed(history):
        ...
        parsed = parse_wecom_todo_request(content, skill_explicit=True)
        if parsed and parsed.title:
            return parsed.title
    return ""


def _finalize_delete(title_query: str, assignee_names: list[str] | None = None) -> ParsedWeComTodoDeleteRequest | None:
    title_query = re.sub(r"\s+", " ", (title_query or "").strip(" ：:，,。.;；"))
    names = [name for name in (assignee_names or []) if name]
    if not title_query and not names:
        return None
    return ParsedWeComTodoDeleteRequest(title_query=title_query[:120], assignee_names=names)


def parse_wecom_todo_delete_request(
    message: str,
    *,
    known_names: list[str] | None = None,
    history: list[dict] | None = None,
    skill_explicit: bool = False,
) -> ParsedWeComTodoDeleteRequest | None:
    raw = _clean_message(message)
    if not raw:
        return None
    raw = re.sub(
    ...
    patterns = [
        r"^(?:删除|移除|撤销|去掉|取消|删掉|清除)(?:企业微信|企微)?待办[：:\s]*(?P<body>.+)$",
        r"^(?:把|将)(?P<body>.+?)(?:的)?(?:企业微信|企微)?待办(?:删除|移除|撤销|删掉|清除)$",
        r"^(?:删除|移除|撤销|去掉|取消|删掉|清除)(?P<body>.+)$",
    ]
    body = ""
    ...
    if not body and _looks_like_delete_intent(raw, history):
        body = _strip_delete_prefix(raw)

    if not body or _VAGUE_DELETE_RE.search(body) or len(body) <= 4 and _VAGUE_DELETE_RE.search(raw):
        body = _extract_todo_title_from_history(history) or body

    if body and known_names:
        names, title = _match_known_names(body, known_names)
        if names and title:
            return _finalize_delete(title, names)
        if names and not title:
            return _finalize_delete("", names)

    if body:
        names, title = _split_name_title_body(body)
        if names and title:
            return _finalize_delete(title, names)
        cleaned = _FILLER_RE.sub(" ", body)
        cleaned = re.sub(r"\s+", " ", cleaned).strip(" ：:，,。.;；")
        if cleaned and cleaned not in {"企业微信", "企微", "待办", "任务"}:
            return _finalize_delete(cleaned, [])

    if skill_explicit or _looks_like_delete_intent(raw, history):
        history_title = _extract_todo_title_from_history(history)
        if history_title:
            return _finalize_delete(history_title, [])
    return None


def extract_wecom_todo_delete_intent_with_llm(
    *,
    user,
    message: str,
    history: list[dict] | None = None,
    ...
    prompt = (
        ...
        f"【通讯录候选姓名】\n"
        f"{('、'.join(candidates) if candidates else '（无）')}\n\n"
        "请输出 JSON。"
    )
    try:
        ...
        logger.exception("wecom todo delete LLM intent extraction failed")
        return None
...
def _find_matching_todo_groups(
    *,
    user,
    organization,
    title_query: str,
    ...
    rows = list(
        WorkTodo.objects.filter(organization=organization, creator=user)
        .select_related("wecom_contact", "assignee")
    ...
    if assignee_names:
        wanted = set(assignee_names)
        rows = [
            row for row in rows
            if row.recipient_name in wanted
            or (row.wecom_contact and row.wecom_contact.name in wanted)
            or (row.assignee and (
                (row.assignee.get_full_name() or "").strip() in wanted
                or row.assignee.username in wanted
            ))
        ]
    grouped: dict = {}
    for row in rows:
        grouped.setdefault(row.sync_group_id, []).append(row)
    return grouped


def delete_wecom_todos_for_skill(
    *,
    user,
    title_query: str,
    assignee_names: list[str] | None = None,
) -> dict:
    from .cli_service import WeComCliError
    from .todo_sync_service import delete_work_todo_group

    organization = current_organization(user)
    if not organization:
        return {"ok": False, "action": "delete", "error": "当前账号尚未加入企业。"}

    groups = _find_matching_todo_groups(
        user=user,
    ...
    if not groups:
        return {
            ...
            "error": (
                "没有找到可删除的待办。"
                f"{'请核对标题：' + title_query if title_query else '请说明要删除哪条待办，例如「删除待办：今天要读书」。'}"
            ),
        }
    if len(groups) > 1:
        titles = []
        for group_rows in groups.values():
            titles.append(group_rows[0].title)
        return {
            "ok": False,
            "action": "delete",
            "error": (
    ...
    group_rows = next(iter(groups.values()))
    representative = group_rows[0]
    try:
        result = delete_work_todo_group(representative.sync_group_id)
    ...
    AuditLog.objects.create(
        trace_id=f"work-todo-skill-delete-{representative.public_id.hex[:12]}",
        actor=user.username,
        intent="技能删除企业微信待办",
        action="work.todo.delete.skill",
        payload={
            "organization_id": organization.id,
            "title": representative.title,
            "sync_group_id": str(representative.sync_group_id),
        },
        decision=AuditLog.Decision.ALLOW,
        result={"deleted": bool(result.get("ok")), "weComDeleted": bool(result.get("weComDeleted"))},
    )
    if not result.get("ok"):
        return {
            "ok": False,
            "action": "delete",
            "error": result.get("detail") or "待办删除失败。",
        }
    return {
        "ok": True,
        "action": "delete",
        ...
        "assignee_names": sorted({
            row.recipient_name or (row.wecom_contact.name if row.wecom_contact else "")
            for row in group_rows
            if row.recipient_name or row.wecom_contact_id
        }),
        "deletedCount": result.get("deletedCount", len(group_rows)),
        "weComDeleted": bool(result.get("weComDeleted")),
        "detail": result.get("detail") or "待办已删除。",
    }


def execute_wecom_todo_delete_skill(
    *,
    user,
    message: str,
    history: list[dict] | None = None,
    ...
    organization = current_organization(user)
    if not organization:
        return {"ok": False, "action": "delete", "error": "当前账号尚未加入企业，无法删除待办。"}
    ...
    if not config or not config.configured:
        return {
            "ok": False,
            "action": "delete",
            "error": "当前企业尚未配置可用的企业微信待办能力，请先在「企微绑定」中完成配置。",
        }
    ...
    if not parsed:
        parsed = extract_wecom_todo_delete_intent_with_llm(
            user=user,
            message=message,
            history=history,
            known_names=known_names,
    ...
    if not parsed or (not parsed.title_query and not parsed.assignee_names):
        return {
            "ok": False,
            "action": "delete",
            "error": (
                "还没理解要删除哪条待办。请说明标题或负责人，"
                "例如「删除待办：今天要读书」；若指刚才创建的那条，可说「把刚才那条待办删掉」。"
            ),
        }

    result = delete_wecom_todos_for_skill(
        user=user,
        title_query=parsed.title_query,
        assignee_names=parsed.assignee_names,
    )
    if result.get("ok"):
...
def execute_wecom_todo_skill(
    *,
    ...
    """AI 理解意图 → 平台创建或删除企微待办。"""
    if _looks_like_delete_intent(message, history):
        return execute_wecom_todo_delete_skill(
            user=user,
            message=message,
            history=history,
            skill_explicit=skill_explicit,
        )
    return _execute_wecom_todo_create_skill(
        user=user,
        message=message,
        history=history,
        skill_explicit=skill_explicit,
...
def _execute_wecom_todo_create_skill(
    *,
    user,
    message: str,
    history: list[dict] | None = None,
...
def format_wecom_todo_skill_result(result: dict) -> str:
    ...
    if result.get("ok") and action == "delete":
        recipients = "、".join(result.get("assignee_names") or [])
        wecom_note = "已从企业微信删除。" if result.get("weComDeleted") else "平台待办已删除。"
        return (
            "【企微待办技能执行结果】\n"
            f"- 操作：删除\n"
            f"- 状态：成功\n"
            ...
            + (f"- 负责人：{recipients}\n" if recipients else "")
            + f"- 说明：{result.get('detail') or wecom_note}\n"
            "请用一两句话向用户确认删除结果；只能根据以上结果描述，不要编造未执行成功的删除。"
        )
    if result.get("ok"):
    ...
    operation = "删除" if action == "delete" else "创建"
    return (
        "【企微待办技能执行结果】\n"
        f"- 操作：{operation}\n"
        f"- 状态：失败\n"
        f"- 原因：{result.get('error') or '未知错误'}\n"
        "请直接告诉用户失败原因与下一步，不要编造已成功删除或创建。"
    )
    ...
    *,
    ...
    for skill in skills or []:
        ...
        blocks.append({
            ...
            "error": "" if result.get("ok") else (result.get("error") or ("删除失败" if result.get("action") == "delete" else "创建失败")),
        })
    return blocks
