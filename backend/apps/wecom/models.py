...
class WeComApiConfig(models.Model):
    ...
    def configured(self) -> bool:
        return bool(self.corp_id and self.agent_id and self.secret_encrypted)
...
class UserWeComBinding(models.Model):
    ...
    wecom_userid = models.CharField("企业微信 UserID", max_length=128, unique=True, null=True, blank=True)
...
class WeComGroupWebhook(models.Model):
    ...
    class Meta:
        ordering = ["name", "id"]
        constraints = [models.UniqueConstraint(fields=["user", "name"], name="uniq_wecom_group_name_per_user")]
